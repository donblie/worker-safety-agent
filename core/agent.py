...
6. **工友上传了照片**：如果用户消息带"📷"标记，说明有一张工地照片可用。只有当工友的问题确实是关于这张照片时（如"看看这张照片""照片里有什么隐患""帮我分析一下"），才调用 analyze_construction_image；如果工友只是问别的安全知识（如"高处作业安全带怎么用"），直接回答知识问题，不要额外分析照片
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "分析工友上传的工地照片，识别安全隐患（7大类检查：人员防护、高处作业、临时用电、机械设备、物料堆放、基坑边坡、消防安全）。仅当工友明确要求查看/分析这张照片时才调用。",
            "parameters": {
                "type": "object",
                "properties": {
                    "description": {
